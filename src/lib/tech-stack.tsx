...
/**
 * Calcola lo stile di posizionamento iniziale sul perimetro ellittico.
 * Garantisce che nel primo paint HTML dal server i pianeti siano GIÀ
 * posizionati correttamente sull'orbita anziché collassati al centro (eliminando pop-in e migliorando lo Speed Index).
 */
export function getInitialOrbitStyle(
  index: number,
  total: number = TECH_ITEMS.length,
  rx: number = 540,
  ry: number = 220
): React.CSSProperties {
  ...
  const depth = Math.sin(baseAngle);
  ...
  return {
    transform: `translate3d(calc(-50% + ${x.toFixed(1)}px), calc(-50% + ${y.toFixed(1)}px), 0px) scale(${scale.toFixed(2)})`,
